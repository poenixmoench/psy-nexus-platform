...
import { MATHEMATICAL_CONSTANTS as CONST } from '../utils';

export const SACRED_GEOMETRY: Record<string, IExtendedGeometricForm> = {
  FLOWER_OF_LIFE: {
  ...
  SRI_YANTRA: {
    ...
    calculatePoints: (size = 1) => {
      ...
      for (let i = 1; i <= 9; i++) {
        ...
        pts.push(
          { x: 0, y: direction * scale, z: 0 },
          { x: -scale, y: -direction * (scale / 2), z: 0 },
          { x: scale, y: -direction * (scale / 2), z: 0 }
        );
      }

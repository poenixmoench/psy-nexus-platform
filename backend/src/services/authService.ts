...
export async function registerUser(username: string, email: string, password: string): Promise<string | null> {
  try {
    ...
    const newUserId = await createUser(username, email, passwordHash); // Korrigiert: Verwende createUser und speichere die ID
    ...
    const token = jwt.sign({ id: newUser.id, email: newUser.email, username: newUser.username }, jwtSecret, { expiresIn: '1h' }); // Korrigiert: Schließende Klammer und Anführungszeichen hinzugefügt

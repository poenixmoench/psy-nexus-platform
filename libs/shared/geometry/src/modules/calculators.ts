...
import { GeometryCache, MATHEMATICAL_CONSTANTS as CONST } from '../utils';
...
export const GEOMETRY_CALCULATORS = {
  ...
  compressPoints: (points: IGeometryPoint[], tolerance: number = 0.1): IGeometryPoint[] => {
    ...
    if (maxDist > tolerance) {
      const left = GEOMETRY_CALCULATORS.compressPoints(points.slice(0, maxIndex + 1), tolerance);
      const right = GEOMETRY_CALCULATORS.compressPoints(points.slice(maxIndex), tolerance);
  ...
  calculateMandalaSymmetry: (baseShape: IGeometryPoint[], repetitions: number = 8): IGeometryPoint[] => {
    const pts: IGeometryPoint[] = [];
    const angleStep = (2 * CONST.PI) / repetitions;

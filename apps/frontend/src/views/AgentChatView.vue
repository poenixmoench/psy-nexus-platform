<template>
  <div class="agent-chat-view p-4 max-w-4xl mx-auto">
    ...
    <div v-if="isLoading" class="mt-4 p-3 bg-green-100 text-green-800 rounded-lg text-sm">
      Agent is processing: Live WebSocket connection established. Watching for updates...
    </div>
  </div>
</template>
...
const sendMessage = async () => {
  ...
  try {
    const response = await fetch('http://localhost:3001/api/agents/CodeAnalyzerAgent/execute', {
      method: 'POST',
      headers: {

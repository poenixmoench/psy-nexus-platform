...
class ContextManager {
    static calculateDelta(allTags, agentNamespace) {
        const NOW = Date.now();
        ...
        const filtered = allTags.filter(tag => {
            if (tag.priority === 'HIGH')
                return true;
            if (tag.namespace === agentNamespace)
                return true;
...
ContextManager.deltaCache = new Map();
ContextManager.RELEVANCE_WINDOW = 2 * 60 * 60 * 1000;
ContextManager.MAX_TAGS = 40; // Hard Limit gegen Token-Explosion
